...
from inspark_infer.ops.compile import CompiledOp, CompileBank, signature
...
class ReferenceBackendTests(unittest.TestCase):
    def plan(self, name="sm89_eager_fp32"):
        return json.loads((Path(__file__).resolve().parents[1]/"configs/hardware/sm89"/(name+".json")).read_text())
