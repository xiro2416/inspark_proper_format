...
构建与缓存分开；engine、ONNX、权重不进 Git。发布前由权利人逐源确认原权重和派生 engine 可再分发，准备 `distribution_attestation.json`：`{"schema":1,"reviewed":true,"redistribution_permitted":true,"sources":{"<configs/common/model_sources.json 中每个 repo>":{"permitted":true,"evidence":"许可依据"}}}`。缺任一源会拒绝上传。只允许私有模型仓库；token 只从 `HF_TOKEN` 环境读取，不放入命令参数、清单或 Git。
