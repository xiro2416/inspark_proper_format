...
The installable package is `src/inspark_infer`. This checkout contains
...
`inspark_infer.cli`, `inspark_infer.config` and
`inspark_infer.models.loader` retain thin public compatibility exports. Old
...
remain in repository-root `models/`, `artifacts/`, `.venv/`, and `.toolchains/`.
Config paths are resolved relative to their configuration file: common settings
are under `configs/common/`, current SM89 deployments under
`configs/hardware/sm89/`, and historical SM120 plans under
`configs/hardware/sm120/`. The runner works from the repository root. Do not
relocate large artifacts into the source package or treat moved historical
plans as newly verified.

From repository root:

```bash
CUDA_VISIBLE_DEVICES='' bash scripts/run.sh -m unittest discover -s tests
CUDA_VISIBLE_DEVICES='' bash scripts/run.sh -m inspark_infer.cli --help
```
