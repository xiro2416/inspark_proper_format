...
def identity():
    from inspark_infer.ops.triton.ar_pipeline.deploy import identity as parent
...
def install(engine,data,*,experimental=False):
    ...
    from inspark_infer.ops.triton.target_seven.projections import install as projections
    from inspark_infer.ops.triton.target_seven.pointwise import PlannedPointwise
    from inspark_infer.ops.triton.target_seven.attention import AttentionPolicy
    result={}
    if data.get('projections',{}).get('choices'):
        result['projections']=projections(engine,data['projections'])
    changes=[]
    if data.get('exact_layernorm'):
        from inspark_infer.ops.cuda.target_seven.exact_norm import prepare as prepare_exact, layernorm as exact_layernorm
        prepare_exact()
        import torch
