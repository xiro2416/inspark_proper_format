...
class EuclideanCodebook(nn.Module):
    ...
    def __init__(self, dim, codebook_size, kmeans_init=False, kmeans_iters=10, decay=0.8, eps=1e-05, threshold_ema_dead_code=2, weight_init=False):
        super().__init__()
        self.decay = decay
...
class VectorQuantize(nn.Module):
    ...
    def __init__(self, input_dim, codebook_size, codebook_dim, commitment=0.005, codebook_loss_weight=1.0, use_l2_normlize=False, codebook_type='euclidean', kmeans_init=False, kmeans_iters=10, decay=0.8, eps=1e-05, threshold_ema_dead_code=2, weight_init=False):
        super().__init__()
        ...
        if self.codebook_type == 'euclidean':
            self.codebook = EuclideanCodebook(self.codebook_dim, codebook_size=self.codebook_size, kmeans_init=self.kmeans_init, kmeans_iters=self.kmeans_iters, decay=self.decay, eps=self.eps, threshold_ema_dead_code=self.threshold_ema_dead_code, weight_init=self.weight_init)
        elif self.codebook_type == 'simple':
            self.codebook = SimpleCodebook(self.codebook_dim, codebook_size=self.codebook_size, use_l2_normlize=self.use_l2_normlize)
        else:
