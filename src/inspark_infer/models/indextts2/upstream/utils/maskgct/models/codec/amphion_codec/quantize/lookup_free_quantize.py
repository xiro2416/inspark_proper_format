...
class LookupFreeQuantize(nn.Module):
    ...
    def forward(self, z):
        ...
        bits = 2 ** torch.arange(self.codebook_dim, device=z.device).unsqueeze(0).unsqueeze(-1).long()
        indices = (torch.round(z_e.clone().detach()).long() * bits).sum(1).long()
        return (z_q, commit_loss, codebook_loss, indices, z_e)

    def vq2emb(self, vq, out_proj=True):
        emb = torch.zeros(vq.shape[0], self.codebook_dim, vq.shape[-1], device=vq.device)
