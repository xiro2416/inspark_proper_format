...
def prepare(engine,plan):
    ...
    try:
        ...
        if plan.get('tensorrt_plan'):
            from acc_infer_clear.tensorrt_backend.deploy import prepare as prepare_tensorrt
        ...
        if plan.get('cfm_triton_fusions'):
            from acc_infer_clear.kernels.stage2_fusions import install as install_cfm_fusions
        ...
        if plan.get('tensorrt113_target_full_plan'):
            from acc_infer_clear.tensorrt_backend.native113 import NativeTargetFullBank113
            with engine.torch.cuda.stream(engine.model.stream),engine.torch.inference_mode():
