...
def pack_weight(weight):
    """N,K FP32/BF16 -> K,N FP8 + N scales, prepared once."""
    w=weight.detach().float();scale=(w.abs().amax(1)/448).clamp_min(1e-12)
    n,k=w.shape
    packed=torch.zeros(k,triton.cdiv(n,32)*32,device=w.device,dtype=torch.float8_e4m3fn)
    packed[:,:n].copy_((w/scale[:,None]).to(torch.float8_e4m3fn).t())
    return packed,scale

def linear(x,packed,scales,bias=None,tile=None,return_kernel=False):
    from .planner import Tile
