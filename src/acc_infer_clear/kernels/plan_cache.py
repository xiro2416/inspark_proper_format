...
from .planner import Tile,DeviceCaps
...
class PlanCache(dict):
    def __init__(self,engine,group):
        ...
        sources={str(p.relative_to(root)):hashlib.sha256(p.read_bytes()).hexdigest()
                 for p in sorted((root/'kernels').glob('*.py'))}
        sources['models/precision.py']=hashlib.sha256((root/'models/precision.py').read_bytes()).hexdigest()
