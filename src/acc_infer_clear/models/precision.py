"""Explicit offline linear-only precision experiment, never called by inference.

Convolution/RNN/KV quantization is NOT implied by this initial linear coverage.
...
from acc_infer_clear.kernels.planner import DeviceCaps,OpSignature,candidates
from acc_infer_clear.kernels.fp8 import pack_weight,linear

class MatrixConv(nn.Module):
    def __init__(self,module,precision,caps,plans):
        super().__init__();self.precision=precision;self.transpose=isinstance(module,nn.ConvTranspose1d)
        ...
        if precision=='bf16':
            ...
            return
        from acc_infer_clear.kernels.fp8_conv import conv1d
    ...
    def forward(self,x):
        if self.precision=='bf16':
            if self.transpose:
                y=torch.nn.functional.conv_transpose1d(x.bfloat16(),self.weight,self.bias,self.stride,self.padding,self.output_padding,self.groups,self.dilation)
            else:y=torch.nn.functional.conv1d(x.bfloat16(),self.weight,self.bias,self.stride,self.padding,self.dilation,self.groups)
            return y.to(x.dtype)
        from acc_infer_clear.kernels.fp8_conv import conv1d
        extent=next((n for n in self.tiles if x.shape[0]*x.shape[-1]<=n),4096)
        return conv1d(x,self.weight,self.scales,self.bias,self.in_channels,self.out_channels,self.kernel_size[0],
                      self.stride[0],self.padding[0],self.dilation[0],self.transpose,self.output_padding[0],self.tiles[extent])

class MatrixLinear(nn.Module):
    def __init__(self,module,precision,caps,plans,extents=(8,64,256,1024,4096)):
    ...
    def forward(self,x):
        if self.precision=='bf16':
            return torch.nn.functional.linear(x.bfloat16(),self.weight,self.bias).to(x.dtype)
        m=x.numel()//self.in_features
        extent=next((n for n in self.tiles if m<=n),self._last_extent)
        return linear(x,self.weight,self.scales,self.bias,self.tiles[extent])
...
def prepare(engine,mode,components,convolutions=False):
    ...
    if convolutions and mode=='fp8':
        from acc_infer_clear.kernels.fp8_conv import prepare_scale_kernels
