...
import torch,triton
from .precision import MatrixConv,MatrixLinear
from .stage2_conv import LayoutConv
from acc_infer_clear.kernels.planner import Tile,DeviceCaps
from acc_infer_clear.kernels.fp8 import linear
from acc_infer_clear.kernels.stage2_gemm import col_linear
from acc_infer_clear.kernels.stage2_bf16_conv import bf16_conv

def identity():
    root=Path(__file__).resolve().parents[1]
    files=['kernels/stage2_gemm.py','kernels/stage2_bf16_conv.py','kernels/stage2_fusions.py','models/stage2_conv.py']
...
def prepare(engine,plan_path,parts=('gemm','conv','norm','gate','rope')):
    ...
    from acc_infer_clear.kernels.stage2_fusions import install
