...
from acc_infer_clear.audio.boundary import head_ready,head_window,HOP,RIGHT
...
class StreamingCore:
    ...
    def prepare_rows(self, sessions, index):
        ...
        from acc_infer_clear.dspark.core import BatchRow
        ...
        requests = []
        for s in sessions:
            s['text'] += s['parts'][index]
            c = s['case']
            requests.append(dict(id=c['id'], text=s['text'], voice_id=c['voice_id'], seed=c['seed'], emotion=c['emotion'], arrival=s['arrival']))
