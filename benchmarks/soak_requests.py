...
def main():
    ...
    parser.add_argument('--deployment',type=workspace_path,default=None,
                        help='Default: matching configs/hardware/sm89/sm89_trt113_safe_b{1,4,8}.json per concurrency tier')
    parser.add_argument('--reference',type=workspace_path,required=True)
    parser.add_argument('--config',type=workspace_path,default=ROOT/'configs/common/runtime_reference.yaml')
    parser.add_argument('--corpus',type=workspace_path,default=ROOT/'configs/hardware/sm89/sm89_quality_256.json')
    ...
    from inspark_infer.runtime.config import load,atomic_json
    ...
    try:
        with GPULease(args.gpu) as lease:
            ...
            for concurrency in args.concurrency:
                deployment_path=args.deployment or ROOT/f'configs/hardware/sm89/sm89_trt113_safe_b{min(concurrency,args.batch)}.json'
