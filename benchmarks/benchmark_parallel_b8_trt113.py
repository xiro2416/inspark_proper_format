...
def main():
    ...
    parser.add_argument("--deployment", default="configs/hardware/sm89/sm89_bf16_trt113_full_b8.json")
    ...
    with GPULease(args.gpu):
        try:
            ...
            def run_wave(prefix, capture_quality=False, concurrent=True):
                ...
                barrier = threading.Barrier(len(lanes)) if concurrent else None
                ...
                pairs = list(zip(lanes, identifiers))
                lane_events = (list(active_executor.map(advance, pairs)) if concurrent else
                               [advance(pair) for pair in pairs])
            ...
            for target_lanes in range(1, args.max_lanes + 1):
                ...
                # The device-side path intentionally owns lane-local batch RNG
                # streams.  Older lanes have already executed more waves than a
                # newly added lane, so align those streams only for the paired
                # same-seed quality check.  Performance waves then continue
                # independently from the resulting per-lane states.
                def align_lane_rng():
                    for lane in lanes:
                        lane.rt.proposal.batch_generator.manual_seed(0x5EED5EED)
                        residual_generator = getattr(lane.rt.residual, "batch_generator", None)
                        if residual_generator is not None:
                            residual_generator.manual_seed(0x7E517E51)

                align_lane_rng()
                sequential_quality_wave = run_wave(
                    f"quality-sequential-n{target_lanes}", capture_quality=True, concurrent=False)
                align_lane_rng()
                quality_wave = run_wave(f"quality-n{target_lanes}", capture_quality=True)
                sequential_reference = sequential_quality_wave["quality"]
                comparisons = []
                for lane_rows in quality_wave["quality"]:
                    for row, (codes, pcm) in enumerate(lane_rows):
                        lane_index = len(comparisons) // 8
                        expected_codes, expected_pcm = sequential_reference[lane_index][row]
                        comparisons.append({
                ...
                cross_lane_sequential = []
                reference = sequential_reference[0]
                for lane_rows in sequential_reference:
                    for row, (codes, pcm) in enumerate(lane_rows):
                        expected_codes, expected_pcm = reference[row]
                        cross_lane_sequential.append(codes == expected_codes and
                                                     bool(np.array_equal(pcm, expected_pcm)))
                quality = {"reference": "each lane's RNG-aligned sequential result",
                           ...
                           "sequential_lanes_all_equal": all(cross_lane_sequential)}
