...
def install_b8_only_graph_policy():
    ...
    from inspark_infer.runtime import graph_policy, graphs, prefix_graphs
    from inspark_infer.runtime.indextts2 import proposal, slot_draft, slot_target
...
def install_shared_engine_classes():
    ...
    import json
    import torch
    from inspark_infer.ops.tensorrt import native113 as native
    ...
    class SharedCFM(base_cfm):
        def __init__(self, plan_path, eager):
            ...
            if engine_sha256 != plan.get("sha256"):
                raise ValueError("TensorRT 11.3 CFM engine hash mismatch")
            self.batch = 8; self.frames = 310; self.component = "cfm"; self.plugins = []
            self.plan = str(plan_file); self.engine_sha256 = engine_sha256
            self.precision = plan.get("precision", "BF16 learned matrices; FP32 interfaces and solver accumulation")
            self.provenance = native._acoustic_provenance_metadata(
                plan_file, plan, "cfm", self.engine_sha256)
            self.input_signature = (
                ("x", (8, 80, 310), torch.float32),
                ("prompt", (8, 80, 310), torch.float32),
                ("lengths", (8,), torch.int64),
                ("style", (8, 192), torch.float32),
                ("mu", (8, 310, 512), torch.float32),
                ("mask", (8, 1, 310), torch.bool),
            )
    ...
    class SharedVocoder(base_vocoder):
        def __init__(self, plan_path, eager):
            ...
            self.batch = 8; self.frames = 52; self.component = "vocoder"
            ...
            self.provenance = native._acoustic_provenance_metadata(
                plan_file, plan, "vocoder", self.engine_sha256)
            self.input_signature = (("mel", (8, 80, 52), torch.float32),)
            ...
            self.output = torch.empty(8, 1, 13312, device=self.device, dtype=torch.float32)
