...
| 检查 | 结果 | 证据 / 限制 |
| --- | --- | --- |
| 根项目 CPU 回归 | 219 passed, 4 skipped | `CUDA_VISIBLE_DEVICES='' ACC_TRITON_TOOLCHAIN=default bash scripts/run.sh -m pytest -q tests` |
...
| 私有 HF publish/fetch | 离线私有上传、公开仓库拒绝、版本固定与缺失 TRT 环境拒绝测试完成 | 尚无逐源再分发许可声明、环境变量 token、完整 bundle 和新服务器复验；未上传 |
