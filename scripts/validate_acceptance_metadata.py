    ...
    try:
        with lease:
            ...
            report['source_hashes']={str(path.relative_to(ROOT)):file_hash(path) for path in (
                Path(__file__),ROOT/'src/inspark_infer/ops/triton/acceptance.py',
                ROOT/'src/inspark_infer/runtime/indextts2/batch_pcg.py',
                ROOT/'src/inspark_infer/models/indextts2/dspark/logic.py')}
            candidate=prefix=None
            if not args.reference_only:
                from inspark_infer.ops.triton.acceptance import acceptance,prefix_plan
