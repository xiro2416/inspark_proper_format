...
def validate(args, report):
    ...
    from inspark_infer.ops.tensorrt.native113 import NativeTargetFull113, NativeDraftFull113
...
def parse_args(argv=None):
    ...
    parser.add_argument("--config", default=str(ROOT / "configs/common/runtime.yaml"))
...
def main(argv=None):
    ...
    report = dict(schema=1, batch=args.batch, physical_gpu=args.gpu, stage="preflight", pass_gate=False,
                  ...
                  script_sha256=digest(__file__), metrics_sha256=digest(ROOT / 'src/inspark_infer/guardrails/numerics.py'))
    error = None
    try:
