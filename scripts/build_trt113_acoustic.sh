#!/usr/bin/env bash
# Sequential B1/B4/B8 acoustic build. Existing exporters retain their GPU leases.
set -euo pipefail
...
bash scripts/run.sh scripts/export_trt113_cfm_onnx.py --gpu "$gpu" --batch "$batch" \
  --config configs/common/runtime_reference.yaml --frames 310 --prompt-frames 258 \
...
bash scripts/run.sh scripts/export_trt113_vocoder_onnx.py --gpu "$gpu" --batch "$batch" \
  --config configs/common/runtime_reference.yaml --frames 52 --regular-conv native \
