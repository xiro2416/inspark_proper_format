...
from inspark_infer.guardrails.snapshots import cpu_copy, load_bundle, save_bundle, write_json
...
def main():
    ...
    parser.add_argument("--config", default="configs/common/runtime_reference.yaml")
    commands = parser.add_subparsers(dest="command", required=True)
    cap = commands.add_parser("capture")
    cap.add_argument("--deployment", required=True)
    cap.add_argument("--corpus", type=Path, default=Path("configs/hardware/sm89/sm89_quality_256.json"))
