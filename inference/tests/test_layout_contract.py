...
from acc_infer_clear import config
from acc_infer_clear.runtime import config as runtime_config
...
class LayoutContractTest(unittest.TestCase):
    ...
    def test_runtime_assets_remain_outside_source_distribution(self):
        values = config.load(ROOT / "configs/runtime.yaml")
    ...
    def test_acoustic_plans_resolve_to_repository_artifacts(self):
        for batch in (1, 4):
            plan = load_deployment(ROOT / f"configs/sm89_bf16_trt113_full_b{batch}.json")
    ...
    def test_cuda_extension_sources_and_notices_are_package_resources(self):
        cases = {
            "acc_infer_clear.ops.cuda.target_seven": ("exact_norm.cu", "LICENSE.pytorch"),
            "acc_infer_clear.ops.cuda.target_norm_quant": ("fused.cu", "NOTICE.md"),
        }
    ...
    def test_sm89_safe_trt_profiles_are_request_isolated_and_batch_specific(self):
        ...
        for batch in (1, 4, 8):
            with self.subTest(batch=batch):
                plan = load_deployment(ROOT / f"configs/sm89_trt113_safe_b{batch}.json")
