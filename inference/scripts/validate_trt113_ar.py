...
def validate(args, report):
    ...
    from acc_infer_clear.ops.tensorrt.native113 import NativeTargetFull113, NativeDraftFull113
...
def main(argv=None):
    ...
    report = dict(schema=1, batch=args.batch, physical_gpu=args.gpu, stage="preflight", pass_gate=False,
                  ...
                  script_sha256=digest(__file__), metrics_sha256=digest(ROOT / 'src/acc_infer_clear/guardrails/numerics.py'))
    error = None
    try:
