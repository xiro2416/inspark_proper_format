...
from acc_infer_clear.guardrails.quality import paired_quality
...
def evaluate(args):
    ...
    try:
        ...
        report["same_loader_checkpoint_identity"] = False
        for component in ("target", "draft", "cfm", "vocoder"):
            a = report["generation"]["baseline"]["model_provenance"][component]["model_sources"]
            b = report["generation"]["candidate"]["model_provenance"][component]["model_sources"]
            if not a or len({row["role"] for row in a}) != len(a) or len({row["role"] for row in b}) != len(b):
                raise ValueError("Invalid model-source role inventory")
            if {row["role"]: row["sha256"] for row in a} != {row["role"]: row["sha256"] for row in b}:
