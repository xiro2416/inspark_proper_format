...
import time
from acc_infer_clear.models.indextts2.streaming import StreamingCore
from acc_infer_clear.runtime.splitter import Splitter
from acc_infer_clear.models.indextts2.audio import head_ready

class Engine(StreamingCore):
    def __init__(self,config):
        import torch
        from acc_infer_clear.models.indextts2.loader import Model
    ...
    def validate_request_isolation(self,strict=False):
        ...
        if getattr(self,'device_round_b8',False):shared.append('device_round_b8')
        changed=[]
        ...
        self.rng_policy='legacy_shared' if shared else ('legacy_device_residual' if changed else 'legacy_per_request')
        return dict(strict_request_isolation=self.strict_request_isolation,rng_policy=self.rng_policy,
                    shared_rng_paths=shared,changed_sampler_paths=changed,request_stream_v1_implemented=False)
    ...
    def prepare_precision(self,mode,components,convolutions=False):
        from acc_infer_clear.quantization.precision import prepare
    ...
    def prepare_acoustic_kernels(self,plan_path=None,mode='alias'):
        from acc_infer_clear.ops.triton.acoustic.kernels import prepare
        with self.torch.cuda.stream(self.model.stream),self.torch.inference_mode():
            return prepare(self,plan_path,mode)
    def prepare_acoustic_stage2(self,plan_path):
        from acc_infer_clear.ops.triton.acoustic.stage2 import prepare
    ...
    def prepare_acoustic_pipeline(self,plan_path):
        from acc_infer_clear.ops.triton.acoustic_pipeline.deploy import prepare
        with self.torch.cuda.stream(self.model.stream),self.torch.inference_mode():return prepare(self,plan_path)
    def prepare_acoustic_refine(self,plan_path):
        from acc_infer_clear.ops.triton.acoustic_refine.deploy import prepare
        with self.torch.cuda.stream(self.model.stream),self.torch.inference_mode():return prepare(self,plan_path)
    def prepare_ar_refine(self,plan_path):
        from acc_infer_clear.ops.triton.ar_refine.deploy import prepare
        with self.torch.cuda.stream(self.model.stream),self.torch.inference_mode():return prepare(self,plan_path)
    def prepare_ar_pipeline(self,plan_path):
        from acc_infer_clear.ops.triton.ar_pipeline.deploy import prepare
        with self.torch.cuda.stream(self.model.stream),self.torch.inference_mode():return prepare(self,plan_path)
    def prepare_slot_target(self,graphs=False):
        if self.sessions:raise RuntimeError('Prepare Target before admitting requests')
        from acc_infer_clear.runtime.indextts2.slot_target import SlotTarget
        with self.torch.cuda.stream(self.model.stream),self.torch.inference_mode():
    ...
    def prepare_target_seven(self,plan_path):
        from acc_infer_clear.ops.triton.target_seven.deploy import prepare
        with self.torch.cuda.stream(self.model.stream),self.torch.inference_mode():return prepare(self,plan_path)
    ...
    def prepare_slot_draft(self):
        ...
        from acc_infer_clear.runtime.indextts2.slot_draft import DraftPool, SlotDraft
        with self.torch.cuda.stream(self.model.stream),self.torch.inference_mode():
