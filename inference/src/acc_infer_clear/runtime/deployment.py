...
def prepare(engine,plan):
    from acc_infer_clear.runtime import device
    ...
    try:
        if plan.get('compute_backend') in ('eager','compile'):
            from acc_infer_clear.ops.backend import prepare_reference
            return prepare_reference(engine,plan,result)
        if plan.get('planner_v2_manifest'):
            from acc_infer_clear.ops.planning.v2.deploy import prepare as prepare_planner_v2
        ...
        if plan.get('tensorrt_plan'):
            from acc_infer_clear.ops.tensorrt.deploy import prepare as prepare_tensorrt
            result['tensorrt']=prepare_tensorrt(engine,plan['tensorrt_plan'])
        if plan.get('tensorrt113_cfm_plan'):
            from acc_infer_clear.ops.tensorrt.native113 import NativeCFMSolver113
        ...
        if plan.get('cfm_triton_fusions'):
            from acc_infer_clear.ops.triton.stage2_fusions import install as install_cfm_fusions
        ...
        if plan.get('tensorrt113_vocoder_plan'):
            from acc_infer_clear.ops.tensorrt.native113 import NativeVocoder113
            with engine.torch.cuda.stream(engine.model.stream),engine.torch.inference_mode():
        ...
        if plan.get('target_norm_quant') or plan.get('target_residual_fusion'):
            from acc_infer_clear.ops.target_norm_quant.deploy_experiment import attach as attach_norm_quant
        ...
        if plan.get('full_m_plan'):
            from acc_infer_clear.ops.triton.target_full_m.deploy import prepare as prepare_full_m
        ...
        if plan.get('unified_ar'):
            from acc_infer_clear.ops.triton.unified_ar.deploy import attach_target
        ...
        if plan.get('tensorrt113_draft_full_plan'):
            from acc_infer_clear.ops.tensorrt.native113 import NativeDraftFullBank113
            with engine.torch.cuda.stream(engine.model.stream),engine.torch.inference_mode():
        ...
        if plan.get('draft_qkv_fusion'):
            ...
            from acc_infer_clear.ops.triton.draft_fusion.deploy import prepare as prepare_draft_fusion
            result['draft_qkv_fusion']=prepare_draft_fusion(engine)
        if plan.get('full_m_plan'):
            from acc_infer_clear.ops.triton.target_full_m.deploy import prepare_combined
            result['full_m_combined_qkv']=prepare_combined(engine,plan['full_m_plan'])
        if plan.get('unified_ar'):
            from acc_infer_clear.ops.triton.unified_ar.deploy import attach_draft
        ...
        if plan.get('device_round_b8'):
            ...
            if device_b8_available:
                ...
                from acc_infer_clear.ops.triton.device_commit import prepare as prepare_device_commit
                from acc_infer_clear.runtime.graph_policy import batches as graph_batches
                ...
                if parent:
                    from acc_infer_clear.runtime.indextts2.device_round import DeviceB8GraphBank
        ...
        if plan.get('prefix_padding_plan'):
            from acc_infer_clear.runtime.prefix_padding.deploy import prepare as prepare_padding
