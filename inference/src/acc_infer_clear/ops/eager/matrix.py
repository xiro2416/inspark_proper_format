...
class MatrixLinear(nn.Module):
    def __init__(self, module, precision, caps, plans,
        ...
        super().__init__()
        from acc_infer_clear.quantization.precision import configure_linear
        configure_linear(self, module, precision, caps, plans, extents)

    def forward(self, x):
        from acc_infer_clear.ops.matrix import linear as dispatch
...
class MatrixConv(nn.Module):
    def __init__(self, module, precision, caps, plans):
        super().__init__()
        from acc_infer_clear.quantization.precision import configure_conv
        configure_conv(self, module, precision, caps, plans)

    def forward(self, x):
        from acc_infer_clear.ops.matrix import conv1d as dispatch
        return dispatch(x, self)
