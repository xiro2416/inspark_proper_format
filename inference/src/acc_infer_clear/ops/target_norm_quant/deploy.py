...
import types
import torch
from acc_infer_clear.ops.cuda.target_norm_quant.fused import prepare, fused
from acc_infer_clear.ops.triton.target_norm_quant.prequantized import Prepared, resolve, base_linear, binding
...
def paired_math(self,x,slots,lengths,limit):
    ...
    for index,block in enumerate(model.h):
        ...
        out=self._norm_quant_attention(q,self.storage[index,0],self.storage[index,1],self.keep,slots,lengths,limit)
