...
import torch,triton
from acc_infer_clear.ops.eager.matrix import MatrixConv
from acc_infer_clear.ops.planning.planner import DeviceCaps, Tile
from acc_infer_clear.ops.triton.fp8_conv_ntc import NTCConv

def identity():
    root=Path(__file__).resolve().parents[3]
    files=['ops/triton/fp8_conv_ntc.py','ops/triton/fp8_conv.py','ops/planning/planner.py',
           'quantization/precision.py','quantization/weights.py','ops/eager/matrix.py','ops/matrix.py']
...
def prepare(engine,plan_path,mode='both'):
    ...
    if mode in ('alias','both'):
        from acc_infer_clear.ops.triton.alias_free import install
