...
def identity():
    from acc_infer_clear.ops.triton.ar_pipeline.deploy import identity as parent
    data=parent();folder=Path(__file__).parent
    package_root=Path(__file__).resolve().parents[3]
    source_files=[*folder.iterdir(),*(package_root/'ops/cuda/target_seven').iterdir()]
    data['seven_sources']={str(p.relative_to(package_root)):hashlib.sha256(p.read_bytes()).hexdigest() for p in source_files if p.suffix in ('.py','.cu')}
    src=Path(__file__).resolve().parents[3]/'runtime/indextts2/slot_target.py'
...
def install(engine,data,*,experimental=False):
    ...
    from acc_infer_clear.ops.triton.target_seven.projections import install as projections
    from acc_infer_clear.ops.triton.target_seven.pointwise import PlannedPointwise
    from acc_infer_clear.ops.triton.target_seven.attention import AttentionPolicy
    result={}
    if data.get('projections',{}).get('choices'):
        result['projections']=projections(engine,data['projections'])
    changes=[]
    if data.get('exact_layernorm'):
        from acc_infer_clear.ops.cuda.target_seven.exact_norm import prepare as prepare_exact, layernorm as exact_layernorm
        prepare_exact()
        import torch
