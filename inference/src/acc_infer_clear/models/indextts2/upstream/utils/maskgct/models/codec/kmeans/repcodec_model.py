...
from acc_infer_clear.models.indextts2.upstream.utils.maskgct.models.codec.amphion_codec.quantize import ResidualVQ
from acc_infer_clear.models.indextts2.upstream.utils.maskgct.models.codec.kmeans.vocos import VocosBackbone
...
class RepCodec(nn.Module):
    ...
    def __init__(self, codebook_size=8192, hidden_size=1024, codebook_dim=8, vocos_dim=384, vocos_intermediate_dim=2048, vocos_num_layers=12, num_quantizers=1, downsample_scale=1, cfg=None):
        super().__init__()
        ...
        vocos_intermediate_dim = cfg.vocos_intermediate_dim if cfg is not None and hasattr(cfg, 'vocos_dim') else vocos_intermediate_dim
        ...
        downsample_scale = cfg.downsample_scale if cfg is not None and hasattr(cfg, 'downsample_scale') else downsample_scale
        ...
        self.encoder = nn.Sequential(VocosBackbone(input_channels=self.hidden_size, dim=self.vocos_dim, intermediate_dim=self.vocos_intermediate_dim, num_layers=self.vocos_num_layers, adanorm_num_embeddings=None), nn.Linear(self.vocos_dim, self.hidden_size))
        self.decoder = nn.Sequential(VocosBackbone(input_channels=self.hidden_size, dim=self.vocos_dim, intermediate_dim=self.vocos_intermediate_dim, num_layers=self.vocos_num_layers, adanorm_num_embeddings=None), nn.Linear(self.vocos_dim, self.hidden_size))
