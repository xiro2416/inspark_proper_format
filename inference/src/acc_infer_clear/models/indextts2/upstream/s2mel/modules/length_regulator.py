...
import numpy as np
from acc_infer_clear.models.indextts2.upstream.s2mel.dac.nn.quantize import VectorQuantize
f0_max = 1100.0
f0_min = 50.0
...
class InterpolateRegulator(nn.Module):
    ...
    def __init__(self, channels: int, sampling_ratios: Tuple, is_discrete: bool=False, in_channels: int=None, vector_quantize: bool=False, codebook_size: int=1024, out_channels: int=None, groups: int=1, n_codebooks: int=1, quantizer_dropout: float=0.0, f0_condition: bool=False, n_f0_bins: int=512):
        super().__init__()
    ...
    def forward(self, x, ylens=None, n_quantizers=None, f0=None, length_hint=None):
        if length_hint is not None and (not isinstance(length_hint, int) or length_hint < 1):
            raise ValueError('Invalid length hint')
        if self.training:
        ...
        mask = sequence_mask(ylens, max_length=length_hint).unsqueeze(-1)
        if self.interpolate:
            x = F.interpolate(x.transpose(1, 2).contiguous(), size=ylens.max() if length_hint is None else length_hint, mode='nearest')
        else:
        ...
        if self.f0_condition:
            if f0 is None:
                ...
                f0_emb = F.interpolate(f0_emb.transpose(1, 2).contiguous(), size=ylens.max() if length_hint is None else length_hint, mode='nearest')
                x = x + f0_emb
