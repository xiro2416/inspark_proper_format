...
from acc_infer_clear.models.indextts2.upstream.s2mel.modules.bigvgan import activations
from acc_infer_clear.models.indextts2.upstream.s2mel.modules.bigvgan.utils import init_weights, get_padding
from acc_infer_clear.models.indextts2.upstream.s2mel.modules.bigvgan.alias_free_activation.torch.act import Activation1d as TorchActivation1d
from acc_infer_clear.models.indextts2.upstream.s2mel.modules.bigvgan.env import AttrDict
...
class BigVGAN(torch.nn.Module, PyTorchModelHubMixin, library_name='bigvgan', repo_url='https://github.com/NVIDIA/BigVGAN', docs_url='https://github.com/NVIDIA/BigVGAN/blob/main/README.md', pipeline_tag='audio-to-audio', license='mit', tags=['neural-vocoder', 'audio-generation', 'arxiv:2206.04658']):
    """
    ...
    def __init__(self, h: AttrDict):
        ...
        activation_post = activations.Snake(ch, alpha_logscale=h.snake_logscale) if h.activation == 'snake' else activations.SnakeBeta(ch, alpha_logscale=h.snake_logscale) if h.activation == 'snakebeta' else None
    ...
    def _from_pretrained(cls, *, model_id: str, revision: str, cache_dir: str, force_download: bool, proxies: Optional[Dict], resume_download: bool, local_files_only: bool, token: Union[str, bool, None], map_location: str='cpu', strict: bool=False, use_cuda_kernel: bool=False, **model_kwargs):
        ...
        if os.path.isdir(model_id):
            ...
            model_file = hf_hub_download(repo_id=model_id, filename='bigvgan_generator.pt', revision=revision, cache_dir=cache_dir, force_download=force_download, proxies=proxies, resume_download=resume_download, token=token, local_files_only=local_files_only)
