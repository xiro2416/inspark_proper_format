...
from acc_infer_clear.models.indextts2.upstream.gpt.transformers_gpt2 import GPT2PreTrainedModel, GPT2Model
from transformers.modeling_outputs import CausalLMOutputWithCrossAttentions
from transformers.utils.model_parallel_utils import assert_device_map, get_device_map
from acc_infer_clear.models.indextts2.upstream.gpt.conformer_encoder import ConformerEncoder
from acc_infer_clear.models.indextts2.upstream.gpt.perceiver import PerceiverResampler
from acc_infer_clear.models.indextts2.upstream.utils.arch_util import AttentionBlock
from acc_infer_clear.models.indextts2.upstream.utils.typical_sampling import TypicalLogitsWarper
from acc_infer_clear.models.indextts2.upstream.utils.tokenizer import LANGUAGE_DICT
...
class GPT2InferenceModel(GPT2PreTrainedModel):
    from acc_infer_clear.runtime.state import RequestField as _RequestField
    cached_mel_emb = _RequestField('target.cached_mel_emb')
...
class UnifiedVoice(nn.Module):
    ...
    def __init__(self, layers=8, model_dim=512, heads=8, max_text_tokens=120, max_mel_tokens=250, max_conditioning_inputs=1, mel_length_compression=1024, number_text_tokens=256, start_text_token=0, stop_text_token=1, number_mel_codes=8194, start_mel_token=8192, stop_mel_token=8193, train_solo_embeddings=False, use_mel_codes_as_input=True, checkpointing=True, types=1, condition_num_latent=32, condition_type='perceiver', condition_module=None, emo_condition_module=None, spk_cond_mode='conformer'):
        ...
        if spk_cond_mode == 'campplus':
            ...
            if condition_type == 'perceiver':
                ...
                self.conditioning_encoder = ConformerEncoder(input_size=1024, output_size=condition_module['output_size'], linear_units=condition_module['linear_units'], attention_heads=condition_module['attention_heads'], num_blocks=condition_module['num_blocks'], input_layer=condition_module['input_layer'])
                if condition_type == 'conformer_perceiver':
                    self.perceiver_encoder = PerceiverResampler(model_dim, dim_context=condition_module['output_size'], ff_mult=condition_module['perceiver_mult'], heads=condition_module['attention_heads'], num_latents=self.cond_num)
            else:
        ...
        self.emo_conditioning_encoder = ConformerEncoder(input_size=1024, output_size=emo_condition_module['output_size'], linear_units=emo_condition_module['linear_units'], attention_heads=emo_condition_module['attention_heads'], num_blocks=emo_condition_module['num_blocks'], input_layer=emo_condition_module['input_layer'])
        self.emo_perceiver_encoder = PerceiverResampler(1024, dim_context=emo_condition_module['output_size'], ff_mult=emo_condition_module['perceiver_mult'], heads=emo_condition_module['attention_heads'], num_latents=1)
    ...
    def post_init_gpt2_config(self, kv_cache=False, half=False):
        ...
        gpt_config = GPT2Config(vocab_size=self.number_mel_codes, n_positions=seq_length, n_ctx=seq_length, n_embd=self.model_dim, n_layer=self.layers, n_head=self.heads, gradient_checkpointing=False, use_cache=True)
    ...
    def get_conditioning(self, speech_conditioning_input, cond_mel_lengths=None):
        if self.condition_type == 'perceiver':
            ...
            speech_conditioning_input, mask = self.conditioning_encoder(speech_conditioning_input.transpose(1, 2), cond_mel_lengths)
    ...
    def get_emo_conditioning(self, speech_conditioning_input, cond_mel_lengths=None):
        speech_conditioning_input, mask = self.emo_conditioning_encoder(speech_conditioning_input.transpose(1, 2), cond_mel_lengths)
    ...
    def forward(self, speech_conditioning_latent, text_inputs, text_lengths, mel_codes, mel_codes_lengths, emo_speech_conditioning_latent, cond_mel_lengths=None, emo_cond_mel_lengths=None, emo_vec=None, use_speed=None, do_spk_cond=False):
        """
    ...
    def prepare_gpt_inputs(self, conditional_latents: torch.Tensor, text_inputs: torch.Tensor, langs: torch.Tensor=None):
        """
        ...
        fake_inputs = torch.ones((batched_mel_emb.shape[0], batched_mel_emb.shape[1] + 1), dtype=torch.long, device=device)
        fake_inputs[:, -1] = self.start_mel_token
        return (fake_inputs, batched_mel_emb, attention_mask)

    def inference_speech(self, speech_condition, text_inputs, langs=None, emo_speech_condition=None, cond_lengths=None, emo_cond_lengths=None, emo_vec=None, use_speed=False, campplus_embedding=None, wav=None, input_tokens=None, num_return_sequences=1, max_generate_length=None, typical_sampling=False, typical_mass=0.9, **hf_generate_kwargs):
        """
        Args:
        ...
        if self.spk_cond_mode == 'campplus':
            if campplus_embedding is not None:
                ...
                if not hasattr(self, 'sv_pipeline'):
                    ...
                    self.sv_pipeline = pipeline(task='speaker-verification', model='iic/speech_campplus_sv_zh-cn_16k-common', device='cpu')
        ...
        if input_tokens is None:
            ...
            assert num_return_sequences % input_tokens.shape[0] == 0, 'The num_return_sequences must be divisible by the batch number of input_tokens'
            assert num_return_sequences % text_inputs.shape[0] == 0, 'The num_return_sequences must be divisible by the batch number of text_inputs'
        ...
        if self.accel_engine is not None and num_return_sequences == 1:
            output = self.accel_engine.generate(inputs, max_new_tokens=max_length - trunc_index, attention_mask=attention_mask, temperature=hf_generate_kwargs.get('temperature', 1), stop_tokens=[self.stop_mel_token], tts_embeddings=inputs_embeds, tts_mel_embedding=self.inference_model.embeddings, tts_text_pos_embedding=self.inference_model.text_pos_embedding)
        else:
            output = self.inference_model.generate(inputs, bos_token_id=self.start_mel_token, pad_token_id=self.stop_mel_token, eos_token_id=self.stop_mel_token, attention_mask=attention_mask, max_length=max_length, logits_processor=logits_processor, num_return_sequences=num_return_sequences, **hf_generate_kwargs)
