...
def main():
    ...
    p.add_argument('--config',default='configs/runtime_reference.yaml');p.add_argument('--gpu',type=int,default=0)
    p.add_argument('--workers',type=int,default=1);p.add_argument('--batch',type=int)
    p.add_argument('--deployment',default='configs/sm89_eager_fp32.json',help='Offline deployment JSON; default is same-weight pure FP32 eager')
    ...
    def emit(e):
        print(json.dumps(audio_event(e),ensure_ascii=False),flush=True)
    with GPULease(a.gpu),Pool(cfg,a.gpu,a.workers) as pool:
        ...
        if a.text:
            ...
            for line in sys.stdin:
                event=parse_event(line);op=event['op'];ident=event.get('id')
